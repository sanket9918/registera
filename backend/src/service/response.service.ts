...
import { CreateResponseInput, GetResponseByForm, GetResponseByUser, ResponseModel } from '../schema/response.schema';
import { User } from '../schema/user.schema';
import mongoose from 'mongoose';
import { FormModel } from '../schema/form.schema';
class ResponseService {
  async createResponse(
    input: CreateResponseInput & { user: User['_id'] } & { userEmail: User['email'] } & { userName: User['name'] }
  ) {
    ...
    const responseValidity = await ResponseModel.findOne({ user: input.user, form: input.form });
    if (responseValidity) {
